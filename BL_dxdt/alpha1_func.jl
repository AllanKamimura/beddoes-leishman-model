function alpha1_func(alpha_ss,alpha_ds0,delta_alpha_0,delta_alpha_1,d_cm,d_cc,z_cm,z_cc,R,RD,qR,q,theta,theta_max,RD_m,RD_theta,gamma_LS)
    ...
    if theta*q >= 0
        ...
        P = 0;
        S = 0;
        T = exp(-50*RD_m); # A factor identifying static condition 
    else
        # Downstroke - Delay the reattachment 
        if abs(theta_max) > 1
            P = exp(-gamma_LS*(abs(theta_max)^4-1)); # Determines how light stall is
            S = 1;                                   # Determines if stall has occurred
        else
            P = 0;
            S = 0;
        end
    ...
    alpha1_n = alpha_ss+dalpha1_n; # Unsteady breakpoint of separation angle
    alpha1_m = alpha_ss+dalpha1_m;
    alpha1_c = alpha_ss+dalpha1_c;

    return alpha1_n,dalpha1_n,alpha1_m,dalpha1_m,alpha1_c,dalpha1_c,P,S,T
end