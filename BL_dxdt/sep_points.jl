function sep_points(so_state,f0,fb,alpha,alpha1_n,alpha_ss,S1,S2,R,q,theta,RD,alpha1_m,alpha1_c,theta_max,theta_min,RD_tv0,R_dot,P,T,df0_c,fSig_1n,fSig2_n,fS2_n,fS2_c_up,fS2_c_down)
##########
if abs(theta_max) > 1
...
if abs(so_state) <= alpha1_n
    ...
    if theta*q > 0 # Upstroke
        f1_n = fb-f0_n;
        S2_prime = S2*(1+fS2_n*(1-RD^(1/2))-
        D_S2n+fSig2_n*Sigma2*RD*(1-RD)); # fS2_n*(1-RD^(1/2)) to delay the dettachment at low pitch rates, so as +fSig2_n*Sigma2*RD*(1-RD) [RD*(1-RD) to have greater effect on medium pitch rates, e.g. frames 10117 and 10118], -D_S2n to match rapid c_n drop and slow growth at stall in almost static conditions
...
if abs(so_state) <= alpha1_c
    ...
    return f,fprime_n,fprime_m,fprime_c,Sigma2
end